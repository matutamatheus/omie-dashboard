...
import { getAgingBuckets, getAgingByClient } from '@/lib/data-provider';
import { subDays, format } from 'date-fns';

export async function GET(request: NextRequest) {
  const params = request.nextUrl.searchParams;
  const today = new Date();
  const type = params.get('type') || 'buckets';
  ...
  if (type === 'clients') {
    const data = await getAgingByClient(filters);
    return NextResponse.json(data);
  }

  const data = await getAgingBuckets(filters);
  return NextResponse.json(data);
}
